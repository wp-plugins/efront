...
class eFront_Lesson extends eFront {
	 ...
	 * @return string|eFront_ApiError 
	 * */
	public static function assignToUser($token, $login, $lesson_id, $type) {
		$xml_response = simplexml_load_file(self::$apiBase . "?action=lesson_to_user" . "&token=" . $token . "&login=" . $login . "&lesson=" . $lesson_id . "&type=" . $type);
		if ($xml_response -> status == 'error') {
			throw new eFront_ApiError($xml_response -> message);
		} else {
			return $xml_response -> status;
	 ...
	 * @return string|eFront_ApiError 
	 * */
	public static function removeFromUser($token, $login, $lesson_id) {
		$xml_response = simplexml_load_file(self::$apiBase . "?action=lesson_from_user" . "&token=" . $token . "&login=" . $login . "&lesson=" . $lesson_id);
		if ($xml_response -> status == 'error') {
			throw new eFront_ApiError($xml_response -> message);
		} else {
			return $xml_response -> status;
	 ...
	 * @return string|eFront_ApiError 
	 * */
	public static function activateToUser($token, $login, $lesson_id) {
		$xml_response = simplexml_load_file(self::$apiBase . "?action=activate_user_lesson" . "&token=" . $token . "&login=" . $login . "&lesson=" . $lesson_id);
		if ($xml_response -> status == 'error') {
			throw new eFront_ApiError($xml_response -> message);
		} else {
			return $xml_response -> status;
	 ...
	 * @return string|eFront_ApiError 
	 * */
	public static function deactivateFromUser($token, $login, $lesson_id) {
		$xml_response = simplexml_load_file(self::$apiBase . "?action=deactivate_user_lesson" . "&token=" . $token . "&login=" . $login . "&lesson=" . $lesson_id);
		if ($xml_response -> status == 'error') {
			throw new eFront_ApiError($xml_response -> message);
		} else {
			return $xml_response -> status;
